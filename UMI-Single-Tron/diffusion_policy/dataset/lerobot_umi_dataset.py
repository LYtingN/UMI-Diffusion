"""Feed a LeRobot v2.1 UMI dataset to UMI's SINGLE-ARM policy WITHOUT a .zarr.zip.
...
and ``common/sampler.py``). The only UMI-specific thing is the *source*: the
stock loader opens a ``.zarr.zip`` via ``zarr.ZipStore``. This module swaps that
one step: it reads the LeRobot ``data/*.parquet`` (proprio) + ``videos/**/*.mp4``
(rgb) and packs them into an in-memory ``ReplayBuffer`` with EXACTLY the keys the
single-arm ``umi_single_arm_dit`` task expects, then reuses 100% of UMI's sampler
/ normalizer / ``__getitem__``.

This is the SINGLE-ARM adapter: the LeRobot dataset is bimanual (left + right),
we keep the LEFT arm only and map it to robot0 / camera0 (see AskUserQuestion
decision). The right arm is dropped.

LeRobot (bimanual source)          ->  UMI single-arm ReplayBuffer key
  observation.eef_pose_left  (7)   ->  robot0_eef_pos (3) + robot0_eef_rot_axis_angle (3)
  observation.gripper       [:,0]  ->  robot0_gripper_width (1)   (left channel)
  observation.images.left  (mp4)   ->  camera0_rgb (T,224,224,3) uint8
  (synthesized per episode)        ->  robot0_demo_start_pose (6) = frame-0 pose, broadcast
...
Key facts verified against the actual data / UMI source:
  * eef_pose is pos(3) + quat in WXYZ order (parquet col-3 is the scalar w,
    ...
    (``robot0_eef_rot_axis_angle``), so we convert wxyz -> rotvec.
  * ``action`` is NOT stored: SequenceSampler auto-builds it as
    concat([eef_pos, eef_rot_axis_angle, gripper_width]) = 7-D raw when 'action'
    is absent, which UMI's ``__getitem__`` turns into the 10-D rot6d action.
    Matches the single-arm shape_meta.
  * The single-arm task uses ``robot0_eef_rot_axis_angle_wrt_start``, so
    ``__getitem__`` needs ``robot0_demo_start_pose`` in the replay buffer. The
    bimanual adapter does not build it (bimanual uses wrt0/wrt1 instead), so we
    synthesize it here: the first frame's [pos(3), rotvec(3)] broadcast to T.
  ...
  * Images are center-crop-resized to 224x224 with UMI's own cv2 transform when
    available, else Pillow.

Hydra usage: point the task's dataset ``_target_`` at this class and give it the
LeRobot dataset ROOT dir instead of a .zarr.zip. See
``config/train_diffusion_dit_timm_umi_lerobot_workspace.yaml``.
"""
...
# ---- LeRobot field layout (single-arm: keep LEFT == robot0) ----------------
_POSE_KEY = "observation.eef_pose_left"
_VIDEO_KEY = "observation.images.left"
_GRIPPER_KEY = "observation.gripper"  # (T, 2) -> [:,0] = left/robot0
...
def _read_video_frames(mp4_path: str, expected_len: int, transform) -> np.ndarray:
    ...
    aligned frame-for-frame. A single trailing/leading off-by-one from the codec
...
def build_umi_replay_buffer_from_lerobot(
    ...
    """Read a LeRobot v2.1 UMI dataset dir into an in-memory single-arm UMI ReplayBuffer.
    ...
    for ep in range(n_ep):
        ...
        episode = {}
        pose = np.stack(df[_POSE_KEY].values).astype(np.float32)  # (T,7)
        eef_pos = pose[:, 0:3].astype(np.float32)
        eef_rotvec = _quat_wxyz_to_rotvec(pose[:, 3:7])
        episode["robot0_eef_pos"] = eef_pos
        episode["robot0_eef_rot_axis_angle"] = eef_rotvec

        gripper = np.stack(df[_GRIPPER_KEY].values).astype(np.float32)  # (T,2)
        episode["robot0_gripper_width"] = gripper[:, 0:1].astype(np.float32)

        # demo_start_pose: frame-0 [pos(3), rotvec(3)] broadcast to all T frames.
        # UmiDataset.__getitem__ reads [0] and Slerps relative to it for the
        # *_wrt_start proprioception channel.
        start_pose = np.concatenate([eef_pos[0], eef_rotvec[0]], axis=-1)  # (6,)
        episode["robot0_demo_start_pose"] = np.broadcast_to(
            start_pose, (T, 6)).astype(np.float32).copy()
        ...
        episode["camera0_rgb"] = _read_video_frames(mp4, T, img_tf)
...
class LeRobotUmiDataset(UmiDataset):
    """Single-arm UmiDataset backed by a LeRobot v2.1 dir instead of a .zarr.zip.

    Drop-in for ``diffusion_policy.dataset.umi_dataset.UmiDataset`` in the
    ``umi_single_arm_dit`` task: same shape_meta, pose_repr, val_ratio, etc. Only
    the replay-buffer source differs, so ``get_normalizer`` /
    ``get_validation_dataset`` / ``__getitem__`` are all inherited unchanged.
    """
