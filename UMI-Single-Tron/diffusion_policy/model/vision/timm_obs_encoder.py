...
class TimmObsEncoder(ModuleAttrMixin):
    def __init__(self,
        ...
        # Resolve the backbone's OWN input normalization (mean/std). DINOv3 does
        # NOT use ImageNet stats — resolve_model_data_config returns the correct
        # values per backbone (ImageNet for DINOv2/resnet, DINOv3's own stats for
        # DINOv3). Captured here before the resnet/convnext path rewraps `model`.
        # No ImageNet fallback: if resolution fails we raise rather than silently
        # standardize DINOv3 with the wrong stats (that's the bug we're avoiding).
        img_norm_mean = None
        img_norm_std = None
        if imagenet_norm:
        ...
        for key, attr in obs_shape_meta.items():
            ...
            if type == 'rgb':
                ...
                image_shape = shape[1:]
        # Build a TRAIN pipeline (with stochastic augmentation) and a separate
        # EVAL pipeline. RandomCrop/ColorJitter are torchvision v1 transforms that
        # do NOT respond to model.eval() — left as-is they would randomly crop and
        # color-jitter every inference forward, making val metrics noisy and the
        # deployed policy jitter (different action for the same frame each step).
        # For eval we replace RandomCrop with a deterministic CenterCrop of the
        # same size and drop all stochastic augmentations (ColorJitter, Random*).
        ...
        if transforms is not None and not isinstance(transforms[0], torch.nn.Module):
            ...
            # eval: deterministic center crop + resize, then drop remaining
            # stochastic augmentations from the tail (ColorJitter, Random*).
            eval_transforms = [
                torchvision.transforms.CenterCrop(size=crop_size),
                torchvision.transforms.Resize(size=image_shape[0], antialias=True)
            ] + [t for t in transforms[2:] if not _is_stochastic_transform(t)]
        elif transforms is not None:
            # transforms supplied directly as nn.Module instances: keep them for
            # train, but still strip any stochastic ones for the eval pipeline.
            eval_transforms = [t for t in transforms if not _is_stochastic_transform(t)]

        # Append backbone input normalization LAST: crop/resize/jitter all operate
        # in [0,1] space, then Normalize maps [0,1] -> standardized space the
        # backbone was pretrained on. Previously `imagenet_norm` was accepted but
        # never applied — the raw [0,1] input starved the (unfrozen) DINOv3 of a
        # meaningful signal. See memory umi-single-tron-vision-ignored.
        if imagenet_norm:
        ...
        # If no explicit augmentation config was given, eval mirrors train (either
        # a bare Normalize or Identity) — there is nothing stochastic to strip.
    ...
    def forward(self, obs_dict):
        ...
        for key in self.rgb_keys:
            ...
            # eval() -> deterministic pipeline (CenterCrop, no jitter); train() ->
            # stochastic augmentation. Keeps inference/deployment reproducible.
            tf_map = self.key_transform_map if self.training else self.key_eval_transform_map
            img = tf_map[key](img)
